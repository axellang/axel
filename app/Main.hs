...
import Axel.Haskell.Project(buildProject,formatProject,runProject)
import Axel.Haskell.Stack(axelStackageVersion)
import qualified Axel.Parse.AST as AST
import Axel.Parse.Args(Command(FileCommand,ProjectCommand,Version),FileCommand(ConvertFile,RunFile,FormatFile),ProjectCommand(FormatProject,RunProject),commandParserInfo)
...
app (ProjectCommand projectCommand) = (case projectCommand of {FormatProject -> formatProject;RunProject -> ((>>) buildProject runProject)})
